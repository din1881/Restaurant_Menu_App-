...
# Create a state token to prevent maliciuos requests
...
def editRestaurant(restaurant_id):
    ...
    if restaurantCreator.id != login_session['user_id']:
        flash("You can't edit r.r is edited by %s" % restaurantCreator.name)
        return redirect('/login')
    if request.method == 'POST':
...
def deleteRestaurant(restaurant_id):
    ...
    # Validation
    if restaurantCreator.id != login_session['user_id']:
        flash("You can't dlt r.r is dltd by %s" % restaurantCreator.name)
        return redirect('/login')
    if request.method == 'POST':
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if itemCreator.id != login_session['user_id']:
        flash("You can't edit item.menu item edited by %s" % itemCreator.name
        return redirect('/login')
    if request.method == 'POST':
        if request.form['name']:
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    # Validation
    if itemCreator.id != login_session['user_id']:
        flash("You can't dlt item.menu item dlted by %s" % itemCreator.name)
        return redirect('/login')    
    if request.method == 'POST':
